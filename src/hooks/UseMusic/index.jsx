...
export default function MusicHooks() {
  ...
  const getPubSubInfo = () => {
    PubSub.subscribe('ids', async (msg, data) => {
      ...
      //   lrc: lyrc,
      // })
      const oldarr = JSON.parse(sessionStorage.getItem('arr'))
      oldarr.unshift({
        url: val[0].url,
        name: name,
        artist: artistName,
      ...
      sessionStorage.setItem('arr', JSON.stringify([...oldarr]))
  ...
  useEffect(() => {
    sessionStorage.setItem('arr', JSON.stringify([musicInfo]))
    const ap = new APlayer({
      container: document.getElementById('aplayer'),
      listFolded: true,
      lrcType: 1,
  ...
  return <div id="aplayer" style={{ position: 'fixed', left: 0, right: 0, bottom: 0, margin: 0 }}></div>
}
