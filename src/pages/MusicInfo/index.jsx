...
import { PageHeader, Descriptions, Button, Spin } from 'antd'
...
export default function MusicInfo() {
  ...
  const getLyric = async () => {
    const data = await React.$apis.request('get', '/api/lyric', { id: store.getState()?.data?.id })
    setMusicLyric(data)
  }
  ...
  const play = async () => {
    if (store.getState()?.data?.alg) {
      const musicInfo = await HearFromDisInfo(store.getState()?.data)
      store.dispatch(musicInfo)
    } else if (store.getState()?.data?.al) {
      const musicInfo = await HearFromResultInfo(store.getState()?.data)
      store.dispatch(musicInfo)
    } else if (store.getState()?.data?.bMusic) {
      const musicInfo = await HearFromNewSongInfo(store.getState()?.data)
      store.dispatch(musicInfo)
    }
  }
  ...
  useEffect(() => {
    setTimeout(() => {
      setMusicObj(store.getState()?.data)
      getLyric()
      setShow(false)
    }, 1500)
